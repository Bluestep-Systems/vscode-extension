...
suite("HttpClient Mock Test Suite", () => {
  ...
  test("Mock HTTP client with custom headers", async () => {
    ...
    mockHttp.setMockTextResponse("https://api.example.com/csrf-token", "csrf-token-value", 200, {
  ...
  test("Mock HTTP client with error response", async () => {
    // Set up a mock error
    mockHttp.setMockError("https://api.example.com/error", new Error("Network timeout"));
  ...
  test("Mock HTTP client with POST request", async () => {
    // Set up a mock response
    mockHttp.setMockJsonResponse("https://api.example.com/login", { success: true, token: "session-token" });

    // Make a POST request
    const response = await HttpClient.getInstance().fetch("https://api.example.com/login", {
      method: "POST",
    ...
    assert.strictEqual((lastRequest?.options?.headers as Record<string, string>)?.["Content-Type"], "application/json");
  });
  ...
  test("Mock HTTP client throws error for unconfigured URL", async () => {
    ...
    await assert.rejects(
      ...
      (error: Error) => {
        assert.strictEqual(error.message, "No mock response configured for URL: https://api.example.com/unconfigured");
        return true;
      }
    );
  });
